...
class ShowRequests extends React.Component {
  ...
  render() {
    ...
    for(var j = 0 ; j < this.state.rows.length; j++){
      requestsRows.push(
        this.createRequestsData(j, this.state.rows[j].request_date, this.state.rows[j].request_type_matn, this.state.rows[j].request_status_matn),
      );
    }
    return (
      <React.Fragment>
        ...
        >
            ...
            <Table size="small">
                ...
                <TableRow>
                    ...
                    <TableCell className={classes.font} style={{ textAlign: 'center', fontWeight: 'bold' }}>وضعیت درخواست</TableCell>
                </TableRow>
                </TableHead>
                <TableBody> 
